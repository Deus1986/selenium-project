...
class TestOkBotSendAlbumInAccount:
    allure.title("Test ok bot send album in account")
    ...
    def test_ok_bot_send_album_in_account(self, driver):
        ...
        with allure.step("Нажать на группу 'Учкудук'"):
        ...
        with allure.step("Нажать 'Отправить в группу пользователя' в сообщениях"):
            form_page.click_message_button(OkBotLocators.BUTTON_SEND_IN_ACCOUNT_GROUP)
            time.sleep(2)
            form_page.click_button(OkBotLocators.SCROLL_LAST_ELEMENT_BUTTON)
            time.sleep(1)

        with allure.step("Выбрать аккаунт, в чью группу импортировать - Джордж Ким"):
            form_page.click_message_button(OkBotLocators.BUTTON_DJORDJ_KIM_ACCOUNT_GROUP)
            time.sleep(2)

        with allure.step("Выбрать группу"):
            form_page.click_message_button(OkBotLocators.UCHKUDUK_GROUP_LOCATOR)
            time.sleep(2)
        ...
        with allure.step("Проверить, что альбом добавился в группу аккаунта"):
